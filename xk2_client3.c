...
static void do_C(void){
    // type letters for up to 10 seconds (non-letters ignored by server’s stats)
    printf("Type letters for up to 10 seconds; input is captured live.\n");
    ...
    // send request
    send_line("C");
    if(L==0) send_line(""); else {
        // strip trailing newlines to keep payload tidy
        while(L && (buf[L-1]=='\n' || buf[L-1]=='\r')) buf[--L]='\0';
        send_line(buf);
    }
    read_until_END();
