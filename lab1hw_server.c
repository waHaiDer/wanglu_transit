    ...
    for (;;) {
        ...
        for (char *tok = strtok(tmp, " \t\r\n"); tok; tok = strtok(NULL, " \t\r\n")) {
            for (size_t i = 0; tok[i]; ++i) {
                ...
                if (isdigit(ch)) {
                    numbers++;
                    if (nums_list[0]) strncat(nums_list, " ", sizeof(nums_list)-strlen(nums_list)-1);
                    ...
                    if (letters_shift_list[0]) strncat(letters_shift_list, " ", sizeof(letters_shift_list)-strlen(letters_shift_list)-1);
                    char one[2] = {sh, 0};
        ...
        if (used < 0) { perror("snprintf"); close(cfd); close(srv); exit(1); }

        fputs(out, stdout);
        fflush(stdout);

        if (send_all(cfd, out, (size_t)used) < 0) {
